...
void ParticleSystem::OnRender(Nut::OrthoGraphicCamera& camera)
{
	...
	for (auto& particle : m_ParticlePool)
	{
		...
		Nut::Renderer2D::DrawRotatedQuad({ particle.Position.x, particle.Position.y, 0.4f }, { size, size }, particle.Rotation, color);
	}

	Nut::Renderer2D::EndScene();
}

void ParticleSystem::Emit(const ParticleProps& particleProps)				// 释放和重用当前粒子信息
{
